/**
 * aio-hypa tools — `hypa_shell`, `hypa_read`, `hypa_grep`, `hypa_find`, and
 * `hypa_ls` with TUI renderers, image sniffing, and truncation — vendored from
 ...
 * is a structural optional-field type (aio anti-slop convention). Handlers
 * default absent required params to "" — schema-valid calls behave identically.
 */
...
export function registerHypaTools(pi: PiApi, config: HypaPiConfig) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
			...
			const result = await runHypaCommand(pi, config, command, params.timeoutMs, params.raw, signal);
			return toToolText(result, command, true);
		},
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
			// aio adaptation: explicit mapping replaces upstream's `params as {...}`
			// cast; schema-valid calls behave identically.
