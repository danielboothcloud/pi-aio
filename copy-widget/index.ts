...
export function registerCopyWidget(pi: ExtensionAPI): void {
	async function openPicker(ctx: ExtensionContext): Promise<void> {
		if (ctx.mode !== "tui") {
			ctx.ui.notify(
				"/pick is only available in interactive TUI mode",
				"warning",
			);
			return;
		}
		...
		if (!assistantText) {
			ctx.ui.notify(
				"No assistant response is available to copy from",
				"warning",
			);
			return;
		}
		...
		if (blocks.length === 0) {
			ctx.ui.notify(
				"The last assistant response has no fenced code blocks",
				"warning",
			);
			return;
		}
		...
		try {
			...
			ctx.ui.notify(
				`Copied code block #${selected.ordinal} to the clipboard`,
				"info",
			);
		} catch (error) {
	...
	pi.registerCommand("pick", {
		description:
			"Select a fenced code block from the last assistant response and copy it",
		handler: async (_args, ctx) => openPicker(ctx),
	});

	pi.registerShortcut("ctrl+p", {
		description: "Open the fenced code block picker",
		handler: openPicker,
	});
}
