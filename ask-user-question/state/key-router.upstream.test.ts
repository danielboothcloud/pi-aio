...
describe("routeKey — tab_switch", () => {
	...
	it("vim h/l mirror Shift+Tab/Tab in multi-question mode", () => {
		expect(routeKey("l", makeState(), makeRuntime())).toEqual({
			kind: "tab_switch",
			nextTab: 1,
		});
		expect(routeKey("h", makeState({ currentTab: 1 }), makeRuntime())).toEqual({
			kind: "tab_switch",
			nextTab: 0,
		});
	});
