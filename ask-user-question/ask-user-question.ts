...
import {
	...
	buildToolResult,
} from "./tool/response-envelope.js";
import {
	MAX_OPTIONS,
	MAX_QUESTIONS,
	MIN_OPTIONS,
...
Preview content is rendered as markdown in a monospace box. Multi-line text with newlines is supported. When any option has a preview, the UI switches to a side-by-side layout with a vertical option list on the left and preview on the right. Do not use previews for simple preference questions where labels and descriptions suffice. Note: previews are only supported for single-select questions (not multiSelect).`,
		...
		// User interaction must finish before sibling tool calls begin. Without this,
		// Pi's default parallel execution can run mutations before clarification is
		// answered or open multiple questionnaire overlays at once.
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if ((ctx as { mode?: string }).mode === "rpc" && hasDialogUI(ctx.ui)) {
				return buildQuestionnaireResponse(
					await runRpcQuestionnaire(ctx.ui, typed),
					typed,
				);
			}
			...
			const sessionRef: {
				current:
					| import("./state/questionnaire-session.js").QuestionnaireSession
					| null;
			} = { current: null };
			const overlayHandleRef: {
				current: import("@earendil-works/pi-tui").OverlayHandle | undefined;
			} = {
				current: undefined,
			};
			...
			if (
				...
				removeOverlayInputListener = ctx.ui.onTerminalInput((data) => {
					...
					if (handle.isHidden() && !hasAnnouncedHide) {
						...
						ctx.ui.notify?.(
							`ask_user_question hidden — press ${collapseKey} to reopen`,
							"info",
						);
					}
					return { consume: true };
