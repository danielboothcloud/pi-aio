...
describe("resolveCollapseKey", () => {
	it("returns the default when config has no collapseKey", () => {
		...
		expect(resolveCollapseKey({ collapseKey: undefined })).toBe(
			DEFAULT_COLLAPSE_KEY,
		);
	});
	...
	it("returns the default when collapseKey is empty or whitespace", () => {
		...
		expect(resolveCollapseKey({ collapseKey: "   " })).toBe(
			DEFAULT_COLLAPSE_KEY,
		);
	});
	...
	it("returns the off sentinel unchanged (case-insensitive)", () => {
		...
		expect(resolveCollapseKey({ collapseKey: "OFF" })).toBe(COLLAPSE_KEY_OFF);
		expect(resolveCollapseKey({ collapseKey: "  off  " })).toBe(
			COLLAPSE_KEY_OFF,
		);
	});
	...
	it("falls back to the default for malformed specs", () => {
		...
		expect(resolveCollapseKey({ collapseKey: "+ctrl+]" })).toBe(
			DEFAULT_COLLAPSE_KEY,
		);
		expect(resolveCollapseKey({ collapseKey: "ctrl++" })).toBe(
			DEFAULT_COLLAPSE_KEY,
		...
		expect(resolveCollapseKey({ collapseKey: "ctrl+shift+h" })).toBe(
			"ctrl+shift+h",
		);
	});
	...
	it("accepts named special keys and bare base keys", () => {
		expect(resolveCollapseKey({ collapseKey: "ctrl+pageup" })).toBe(
			"ctrl+pageup",
		);
		expect(resolveCollapseKey({ collapseKey: "Ctrl+PageUp" })).toBe(
			"ctrl+pageup",
		...
		expect(resolveCollapseKey({ collapseKey: "alt+escape" })).toBe(
			"alt+escape",
		);
	});
});
...
describe("loadConfig", () => {
	...
	it("reads a valid JSON config", () => {
		mkdirSync(join(home, ".config", "rpiv-ask-user-question"), {
			recursive: true,
		});
		writeFileSync(
			configPath,
