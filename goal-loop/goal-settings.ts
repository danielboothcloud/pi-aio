...
export function globalSettingsPath(): string {
	return path.join(os.homedir(), ".pi", "agent", "aio-goal.settings.json");
}
...
export function settingsProvenance(
	cwd: string,
): Record<
	keyof Settings,
...
> {
	...
	for (const k of SETTINGS_KEYS) {
		if ((proj as Record<string, unknown>)[k] !== undefined)
			out[k] = { value: (proj as any)[k], source: "project" };
		else if ((glob as Record<string, unknown>)[k] !== undefined)
			out[k] = { value: (glob as any)[k], source: "global" };
...
export function saveSettings(
	...
	const file =
		scope === "global" ? globalSettingsPath() : projectSettingsPath(cwd);
	const current = readSettingsFile(file);
	const next: Record<string, unknown> = { ...current };
	for (const [k, v] of Object.entries(patch)) {
		if (v === undefined)
			delete next[k]; // key=unset removes the key
		else next[k] = v;
	}
