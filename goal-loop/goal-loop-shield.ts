...
export function contractItems(contract: string): string[] {
	return (
		contract
			...
			.map((l) =>
				l.replace(
					/^(?:done when|verify|verified when|verification|done)\s*:\s*/i,
					"",
				),
			)
			...
			.filter((l) => !l.endsWith(":"))
			.filter(
				(l) => !/^(?:done when\s+)?(?:all of\s+)?the following\b/i.test(l),
			)
	);
}
 ...
 */
export function checkRegressionShield(
	report: string,
	contract: string,
): RegressionShieldResult {
	const hasEvidenceBlock = /<evidence>[\t\n\r ]*[\s\S]*?<\/evidence>/i.test(
		report,
	);
...
export function parseAuditorVerdict(output: string): {
	...
	const lastAssistant =
		[...parts]
			.reverse()
			.find((t) => /<\/?(approved|disapproved|impossible)[ />]/i.test(t)) ??
		output;
	const impossibleMatch = /<impossible>([\s\S]*?)<\/impossible>/i.exec(
		lastAssistant,
	);
	return {
