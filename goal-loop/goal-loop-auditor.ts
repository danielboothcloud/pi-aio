...
function buildGoalAuditorPrompt(
	goal: Goal,
	...
	const shieldGaps = [...(goal.auditHistory ?? [])]
		.reverse()
		.find((v) => v.regressionShieldPassed === false)?.regressionShieldMissing;
	return [
...
export async function runGoalCompletionAuditor(args: {
	...
	if (!model) {
		return {
			...
			model: "(unset)",
			thinkingLevel,
			error: "no model (session model also unset)",
		};
	}
	...
	try {
		...
		const unsub = session.subscribe((event) => {
			...
			if (
				anyEvent.type === "error" ||
				anyEvent.error ||
				anyEvent.type === "auto_retry_start"
			) {
				const msg =
					anyEvent.error?.message ?? anyEvent.message ?? anyEvent.errorMessage;
		...
		const usedReadTool = toolCalls.some((c) =>
			["read", "grep", "find", "ls", "bash"].includes(c.name),
		);
		...
		if (approved && !usedReadTool) {
			return {
				...
				thinkingLevel,
				error:
					"Auditor approved without calling any read tool; treated as disapproved.",
			};
		}
		...
		if (approved && args.goal.verificationContract?.trim()) {
			const shield = checkRegressionShield(
				output,
				args.goal.verificationContract,
			);
			if (!shield.passed) {
