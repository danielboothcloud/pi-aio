...
export function isQuotaError(error: string | undefined): boolean {
	...
	return /429|quota|rate.?limit|temporarily|credits?|key limit exceeded|insufficient.?balance|too many requests/i.test(
		error,
	);
}
...
export function parseQuotaError(
	error: string,
	defaultRetryAfterSec = 3600,
): QuotaError {
	let m = error.match(/retry-after:\s*(\d+)/i);
	if (m) {
		const sec = Number(m[1]);
		if (Number.isFinite(sec) && sec >= 0)
			return { raw: error, retryAfterSec: sec, fromUpstream: true };
	}
	m = error.match(
		/retry (?:after|in)\s+(\d+)\s*(s|sec|seconds|m|min|minutes|h|hours?)/i,
	);
	if (m) {
		const n = Number(m[1]);
		...
		if (Number.isFinite(n) && n >= 0)
			return { raw: error, retryAfterSec: n * mult, fromUpstream: true };
	}
	return {
		raw: error,
...
export function isSubagentQuotaResult(
	toolName: string,
	isError: boolean,
	payload: unknown,
): boolean {
