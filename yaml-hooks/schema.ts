// Hook definition schema parsing/validation. Ported from pi-yaml-hooks (MIT).
...
export function parseHookDefinition(
	...
	if (!isRecord(hookDefinition)) {
		return {
			errors: [createError(filePath, "invalid_hook", `hooks[${index}] must be an object.`, `hooks[${index}]`)],
		};
	}
	...
	if (!isHookEvent(event)) {
		return {
			errors: [
				...
				createError(filePath, "invalid_event", `hooks[${index}].event is not a supported hook event.`, `hooks[${index}].event`),
			],
		};
	}
...
function parseHookId(
	...
	if (seenIds.has(id)) {
		return {
			errors: [createError(filePath, "duplicate_hook_id", `hooks[${index}].id duplicates hook id "${id}".`, `hooks[${index}].id`)],
		};
	}
	seenIds.add(id);
	return { id, errors: [] };
...
function parseOverrideTarget(
	...
	if (override === undefined) {
		if (disable !== undefined) {
			return {
				errors: [createError(filePath, "invalid_override", `hooks[${index}].disable requires override.`, `hooks[${index}].disable`)],
				isDisableOverride: false,
			};
		}
		return { isDisableOverride: false, errors: [] };
	}

	if (!isNonEmptyString(override)) {
		return {
			errors: [createError(filePath, "invalid_override", `hooks[${index}].override must be a non-empty hook id.`, `hooks[${index}].override`)],
			isDisableOverride: false,
		};
	}

	if (disable === true) {
		return { targetId: override, isDisableOverride: true, errors: [] };
	}
	...
	if (disable !== undefined) {
		return {
			targetId: override,
			isDisableOverride: false,
			errors: [createError(filePath, "invalid_override", `hooks[${index}].disable must be a boolean.`, `hooks[${index}].disable`)],
		};
	}

	return { targetId: override, isDisableOverride: false, errors: [] };
}
...
export function parseScope(
	...
	if (!isHookScope(scope)) {
		return {
			scope: "all",
			errors: [
				createError(filePath, "invalid_scope", `hooks[${index}].scope must be one of: all, main, child.`, `hooks[${index}].scope`),
			],
		};
	}
	return { scope, errors: [] };
...
export function parseRunIn(
	...
	if (!isHookRunIn(runIn)) {
		return {
			runIn: "current",
			errors: [
				createError(filePath, "invalid_run_in", `hooks[${index}].runIn must be one of: current, main.`, `hooks[${index}].runIn`),
			],
		};
	}
	return { runIn, errors: [] };
...
export function parseAsync(
	...
	const normalized = normalizeAsyncConfig(filePath, asyncValue, index);
		...
		return {
			errors: [
				createError(
					filePath,
					"invalid_async",
					`hooks[${index}].async cannot be true for tool.before events because blocking requires synchronous execution.`,
					`hooks[${index}].async`,
				),
			],
		};
		...
		return {
			errors: [
				createError(
					filePath,
					"invalid_async",
					`hooks[${index}].async cannot be true for session.idle events because idle dispatch must complete before tracked changes are consumed.`,
					`hooks[${index}].async`,
				),
			],
		};
		...
		return {
			errors: [
				createError(
					filePath,
					"invalid_async",
					`hooks[${index}].async cannot be true for user.prompt.submit because context must be ready before the agent turn starts.`,
					`hooks[${index}].async`,
				),
			],
		};
...
function normalizeAsyncConfig(
	...
	if (!isRecord(value)) {
		return {
			enabled: false,
			errors: [
				createError(filePath, "invalid_async", `hooks[${index}].async must be a boolean or { group?, concurrency? }.`, `hooks[${index}].async`),
			],
		};
	}
	...
	if (group !== undefined && !isNonEmptyString(group)) {
		return {
			enabled: false,
			errors: [
				createError(filePath, "invalid_async", `hooks[${index}].async.group must be a non-empty string.`, `hooks[${index}].async.group`),
			],
		};
	}

	const concurrency = value.concurrency;
	if (concurrency !== undefined && (typeof concurrency !== "number" || !Number.isInteger(concurrency) || concurrency <= 0)) {
		return {
			enabled: false,
			errors: [
				createError(
					filePath,
					"invalid_async",
					`hooks[${index}].async.concurrency must be a positive integer.`,
					`hooks[${index}].async.concurrency`,
				),
			],
		};
	}
	...
	if (Object.keys(value).some((key) => key !== "group" && key !== "concurrency")) {
		return {
			enabled: false,
			errors: [
				createError(filePath, "invalid_async", `hooks[${index}].async only supports group and concurrency.`, `hooks[${index}].async`),
			],
		};
	}
		...
		return {
			errors: [
				createError(
					filePath,
					"invalid_hook_action",
					`hooks[${index}].action is only supported on tool.before.* events.`,
					`hooks[${index}].action`,
				),
			],
		};
...
function parseStructuredCondition(
	...
	// `key` is already narrowed to a supported condition key, so branch
	// explicitly instead of laundering a computed key through assertions.
	const parsedCondition: HookCondition =
		key === "matchesAnyPath"
			? { matchesAnyPath: values.values }
			: { matchesAllPaths: values.values };
	return { condition: parsedCondition };
}
...
function normalizePathConditionValues(
	...
	if (!Array.isArray(value)) {
		return {
			error: { message: `${path} must be a non-empty string or non-empty string array.`, path },
		};
	}
	if (value.length === 0) {
		return {
			error: { message: `${path} must not be an empty array.`, path },
		};
	}
...
function supportsPathConditions(event: HookEvent): boolean {
...
export function parseActions(
	...
	if (!Array.isArray(actions) || actions.length === 0) {
		return {
			actions: [],
			errors: [
				createError(filePath, "invalid_actions", `hooks[${index}].actions must be a non-empty array.`, `hooks[${index}].actions`),
			],
		};
	}
...
function parseAction(
	...
	if ("confirm" in action) {
		...
		return confirm
			...
			: {
					errors: [
						createError(filePath, "invalid_action", `${path}.confirm must be { message, title? } with non-empty message.`, `${path}.confirm`),
					],
				};
	}
...
function parseCommandAction(value: unknown): string | HookCommandActionConfig | undefined {
	...
	if (value.args !== undefined && !isNonEmptyString(value.args)) {
		return undefined;
	}
	return { name: value.name, ...(value.args !== undefined ? { args: value.args } : {}) };
}
