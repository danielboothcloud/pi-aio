...
test("find uses FFF glob results before the SDK fallback", async () => {
	...
	const tool = captureTool((pi) =>
		registerFindTool(pi, "/repo", service, sdkResult("sdk.ts", sdkCalls)),
	);
...
test("grep uses FFF only for unscoped searches and preserves SDK scoped search", async () => {
	...
	const tool = captureTool((pi) =>
		registerGrepTool(pi, "/repo", service, sdkResult("sdk:1:hello", sdkCalls)),
	);
...
test("grep registration steers agents toward ripgrep instead of GNU grep", () => {
	const tool = captureTool((pi) =>
		registerGrepTool(
			pi,
			"/repo",
			undefined,
			sdkResult("sdk:1:hello", { count: 0 }),
		),
	) as RegisteredTool & {
		description?: string;
	...
	assert.match(tool.description ?? "", /ripgrep/);
	assert.match(tool.promptSnippet ?? "", /ripgrep/);
	assert.ok((tool.promptGuidelines ?? []).some((g) => /ripgrep/.test(g)));
});
