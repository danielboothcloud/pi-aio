...
function resolveTheme(): BundledTheme {
	...
	try {
		...
		const settings = JSON.parse(
			require("node:fs").readFileSync(require("node:path").join(agentDir, "settings.json"), "utf8"),
		);
...
export function renderGrepResults(text: string, pattern: string): string {
	...
	for (const line of lines) {
		...
		if (fileMatch) {
			...
			const display = highlightLiteralMatches(content, pattern);
...
function highlightLiteralMatches(content: string, pattern: string): string {
	...
	while (offset < content.length) {
		const match = lowerContent.indexOf(lowerPattern, offset);
		if (match < 0) break;
		parts.push(content.slice(offset, match));
		parts.push(`${RST}${FG_YELLOW}\x1b[1m${content.slice(match, match + pattern.length)}${RST}`);
